...
install_prerequisites() {
    brew install dosfstools e2fsprogs
    # TODO: gdisk
...
prepare_efi_partition() {
    ...
    cp ./assets/refind_x64.efi $efi_mount_path/EFI/BOOT/bootx64.efi
    cp ./assets/vmlinuz $efi_mount_path/EFI/BOOT/vmlinuz
    cp ./assets/initrd.img $efi_mount_path/EFI/BOOT/initrd.img
...
prepare_root_partition() {
    ...
    rsync -aP ./assets/busybox $ext4_mount_path/bin/busybox
...
rebuild_drive_file() {
    ...
    rm $header_filename $efi_partition_filename $ext4_partition_filename
}
